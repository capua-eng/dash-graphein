...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function renderAlarms() {
        ...
        currentAlarms.forEach(alarm => {
            ...
            // Se for alarme resolvido (amarelo), não renderiza
            if (!isActive) return;
    ...
    // Botão Confirmar no modal - reconhece o erro via API
    document.getElementById('confirmRecognition').addEventListener('click', async function () {
        if (selectedAlarm) {
            const alarmId = parseInt(selectedAlarm.dataset.id);
            const alarmIndex = currentAlarms.findIndex(a => a.id === alarmId);

            if (alarmIndex !== -1) {
                const alarm = currentAlarms[alarmIndex];
                const recognitionTime = new Date().toISOString();
                
                try {
                    // Atualiza visualmente
                    alarm.originalData.DataErroFim = recognitionTime;
                    alarm.originalData.DataErroRec = recognitionTime;
                    alarm.datetimeOut = formatDateTime(recognitionTime);
                    alarm.isActive = false;
                    renderAlarms();

                    // Envia para o backend
                    await reconhecerAlarmeNoBackend(
                        alarm.originalData.BITS,
                        alarm.originalData.Equipamento,
                        alarm.originalData.DataErroIni
                    );
                    ...
                    alarm.originalData.DataErroFim = null;
                    alarm.originalData.DataErroRec = null;
                    alarm.datetimeOut = '';
                    alarm.isActive = true;
                    renderAlarms();

                    showToast(`Falha ao reconhecer erro: ${error.message}`, 'error');
                }
            }
        }
        recognitionModal.style.display = 'none';
    ...
    // Envia reconhecimento de alarme para a API
    async function reconhecerAlarmeNoBackend(bits, equipamento, dataErroIni) {
        try {
            const response = await fetch('http://192.168.0.252:8080/api/alarmes/reconhecer', {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({
                    ...
                    dataErroIni: dataErroIni
                })
            });

            if (!response.ok) {
                const errorData = await response.json();
                throw new Error(errorData.detail || 'Erro ao reconhecer alarme');
            }
